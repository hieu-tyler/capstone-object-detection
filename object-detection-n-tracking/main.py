...
parser.add_argument("--output_mode", type=str, default="normal",
                    help="output stream type. e.g. 'normal', 'stack', 'debug'")
...
def compare(a, b):
    top_a, bottom_a, left_a, right_a = a['top'], a['bottom'], a['left'], a['right']
    top_b, bottom_b, left_b, right_b = b['top'], b['bottom'], b['left'], b['right']

    # Case 1
    # 1st object is larger than 2nd object
    # both object overlap
    # 2nd object partially surrounded inside 1st object
    if top_a < top_b and bottom_a > bottom_b and left_a < left_b and right_a > right_b:
        return "Overlap case 1"

    # Case 2
    # 1st object is same as 2nd object
    # both object overlap
    if top_a < top_b < bottom_a and left_a < left_b < right_a:
        return "Overlap case 2"

    # Case 3
    # 1st object is same as 2nd object
    # both object overlap
    if top_b < top_a < bottom_b and left_a < left_b < right_a:
        return "Overlap case 3"

    # Case 4
    # 1st object is smaller than 2nd object
    # both object overlap
    if top_b < top_a < bottom_b and top_b < bottom_a < bottom_b and left_b < right_a < right_b and left_a < left_b:
        return "Overlap case 4"
...
while True:
    ...
    cv2.putText(array_img, "Frame number {}".format(frame_number),
                (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 5)
    frame_number += 1
    ...
    for detection in detections:
        ...
        cv2.putText(array_img, str(int(detection.Area)), left,
                    cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 5)

    # --------------------------- HANDLE DATA POINT ----------------------------------------
    data_point_copy = (x for x in data_points)
    object_size = len(data_points)
    message = "null"

    for i in range(object_size):
        for j in range(i + 1, object_size):
            message = compare(data_points[i], data_points[j])
            print("[MESSAGE]", message)
            if message != "null":
                break

        # -------------------- MASK IMAGE FOR TRACKING --------------------------------------------
